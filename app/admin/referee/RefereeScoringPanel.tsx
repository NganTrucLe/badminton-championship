...
export function RefereeScoringPanel({
	...
	const playoffMatches = useMemo(
		() => matches.filter((m) => m.round === 6 || m.round === 7),
		[matches],
	);
	...
	const activeWinner = activeMatch
		? matchWinnerSide(activeMatch.state, activeMatch.sa, activeMatch.sb)
		: null;
	...
	async function commitLive(scoreA: number, scoreB: number) {
		...
		try {
			...
			setMatches((prev) =>
				prev.map((m) =>
					m.id === activeMatch.id ? { ...m, sa: scoreA, sb: scoreB, state: "live" } : m,
				),
			);
	...
	async function commit(state: TMatchState) {
		...
		try {
			...
			setMatches((prev) =>
				prev.map((m) => (m.id === activeMatch.id ? { ...m, sa: draftA, sb: draftB, state } : m)),
			);
			if (state === "done") {
	...
	async function resetMatch() {
		...
		try {
			...
			setMatches((prev) =>
				prev.map((m) => (m.id === activeMatch.id ? { ...m, sa: 0, sb: 0, state: "next" } : m)),
			);
			setConfirmingReset(false);
			setSelectedId("");
						...
						{!activeMatch ? (
							...
							<>
								<div className="flex items-center gap-[10px]">
									<Badge className="ml-auto rounded-full bg-[#F2B544]/15 font-[family-name:var(--font-jetbrains)] text-[10px] tracking-[.12em] text-[#F2B544]">
								...
								<div className="mt-[22px] grid grid-cols-2 gap-[14px]">
									<div
										...
										style={{
											background:
												activeWinner === "a" ? "rgba(63,191,143,.14)" : "rgba(255,253,247,.07)",
											border:
												activeWinner === "a"
													? "1px solid #3FBF8F"
									...
									>
										...
										<div
											className="text-[14px]"
											style={{
												fontWeight: activeWinner === "a" ? 900 : 800,
												color: activeWinner === "a" ? "#FFFDF7" : "#8FBCB0",
											}}
										>
									...
									<div
										...
										style={{
											background:
												activeWinner === "b" ? "rgba(63,191,143,.14)" : "rgba(255,253,247,.07)",
											border:
												activeWinner === "b"
													? "1px solid #3FBF8F"
													: "1px solid rgba(255,255,255,.14)",
									...
									>
										...
										<div
											className="text-[14px]"
											style={{
												fontWeight: activeWinner === "b" ? 900 : 800,
												color: activeWinner === "b" ? "#FFFDF7" : "#8FBCB0",
											}}
										>
										...
										<>
											...
											>
												...
												<AlertDialogContent>
													...
													{resetError && (
														<p className="text-destructive text-[12px] mt-2">{resetError}</p>
													)}
													<AlertDialogFooter>
